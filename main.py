...
# Removing all neighborhoods with fewer than 125 listings. Reduced neighborhood count from 200 to 65.
...
# Removing February and March median rent data did not significantly change the averages.
...
# November and December levels are particularly high, with 11% and 17% of neighborhoods below 5 listings per month.
#Rental activity declines in the winter, likely explaining the decline in listings.
...
percent_missing
#To reduce the rental median variance, removing 4 neighborhoods that fall below the monthly listing thresholds for two distinct months.
...
#For the 61 neighborhoods that pass the overall threshold count, nulling the rental mean data for months with fewer than 5 listings.
...
#Splitting the month range into two subcategories for more granular analysis
...
#Replacing NA with zero so all neighborhoods appear on chloropleth map
